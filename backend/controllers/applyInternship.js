...
const applyInternship = async (req, res) => {
  ...
  const { name, email, contactNo, internshipType, internshipDuration } =
    req.body;
  try {
    if (
      !name ||
...
const updateCertificate = async (req, res) => {
  try {
    ...
    const update = await Apply.findByIdAndUpdate(id, updateData, { new: true });
    if (!update) {
...
const getByCertificate = async (req, res) => {
  try {
    ...
    if (!intern) {
      return res
        .status(404)
        .json({ message: "No intern found with the given certificate number" });
    }
